...
state.  However, you don't want to repeat long running commands each
time if they aren't really needed.  Here is an example of a
~wcdo-local-<name>.rc~ file which will build Wire-Cell Toolkit against
a version of externals from UPS in an idempotent manner.  It handles
all the annoying UPS setup and resetup.
